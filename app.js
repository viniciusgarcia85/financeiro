...
// =============================================
const DIVIDAS_INIT = [
  // Fora da folha — com parcelas mensais
  ...
  { id: 'outros-a',    nome: 'Outros A (mãe)',   total: 13269.90,orig: 13269.90,pago: 0, parcMensal: 221.17,  parcsTotal: 60, parcsPagas: 0, venceDia: 10, urg: 'blue',  tipo: 'fora' },
  // PicPay — folha
...
window.regPagDiv = async () => {
  ...
  const isParcela = document.getElementById('pd-tipo')?.value === 'parcela';
  const div = dividas.find(d => d.id === divId);
  if (!div) return;
  // Se valor 0, usa parcela mensal
  const valorFinal = (!val || val <= 0) ? (div.parcMensal || 0) : val;
  if (!valorFinal || valorFinal <= 0) { toast('Informe um valor válido', 'err'); return; }
  const val2 = valorFinal;
  const novoPago = (div.pago || 0) + val2;
  const novoTotal = Math.max(0, div.total - val2);
  const novasParcsPagas = (div.parcsPagas || 0) + (isParcela ? 1 : Math.floor(val2 / (div.parcMensal || val2)));
  const updates = { pago: novoPago, total: novoTotal, parcsPagas: novasParcsPagas };
  ...
  try {
    ...
    await addDoc(collection(db, 'users', currentUser.uid, 'transacoes'), {
      tipo: 'pag-divida', divida: div.nome, label: 'Pagamento — ' + div.nome, val: val2, data, obs,
      isParcela, criadoEm: Date.now()
    });
    clearForm('pagdiv');
    if (novoTotal <= 0.01) toast('🎉 ' + div.nome + ' QUITADA!');
    else {
      const parcsRest = (div.parcsTotal || 0) - novasParcsPagas;
      toast('✓ ' + fmt(val2) + ' pago em ' + div.nome + '. Restam ' + parcsRest + ' parcelas (' + fmt(novoTotal) + ')');
    }
  } catch (e) { toast('Erro: ' + e.message, 'err'); }
};
...
function bindCatButtons() {
  const btnEntrada = document.getElementById('btn-add-entrada');
  const btnSaida = document.getElementById('btn-add-saida');
  if (btnEntrada) btnEntrada.onclick = () => window.addCat('entrada');
  if (btnSaida) btnSaida.onclick = () => window.addCat('saida');
  // Enter key support
  ['new-cat-entrada','new-cat-saida'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.onkeydown = (e) => {
      if (e.key === 'Enter') window.addCat(id.includes('entrada') ? 'entrada' : 'saida');
    };
  });
}
...
function renderParcelas() {
  const el = document.getElementById('dv-parcelas');
  if (!el) return;
  const hoje = new Date();
  const mesAtual = hoje.getMonth() + 1;
  const anoAtual = hoje.getFullYear();
  const ativas = dividas.filter(d => !d.quitada && d.parcMensal > 0);
  if (!ativas.length) { el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">✅</div><div class="empty-state-text">Nenhuma parcela pendente.</div></div>'; return; }
  // Ordenar por dia de vencimento
  const sorted = [...ativas].sort((a,b) => (a.venceDia||0) - (b.venceDia||0));
  el.innerHTML = `
    <div class="alert al-blue" style="margin-bottom:1rem">
      Parcelas de ${hoje.toLocaleString('pt-BR',{month:'long'})} de ${anoAtual}. 
      Total do mês: <strong>${fmt(sorted.reduce((a,d) => a + d.parcMensal, 0))}</strong>
    </div>
    ${sorted.map(d => {
      const parcsRest = Math.max(0, (d.parcsTotal || 0) - (d.parcsPagas || 0));
      const venceStr = d.venceDia ? `${String(d.venceDia).padStart(2,'0')}/${String(mesAtual).padStart(2,'0')}/${anoAtual}` : d.vence;
      const diasP = d.venceDia ? d.venceDia - hoje.getDate() : 0;
      const urgClass = diasP < 0 ? 'al-red' : diasP <= 5 ? 'al-amber' : 'al-green';
      const urgText = diasP < 0 ? `Vencida há ${Math.abs(diasP)} dias` : diasP === 0 ? 'Vence hoje!' : `Vence em ${diasP} dias`;
      const bc = d.urg === 'red' ? 'b-red' : d.urg === 'amber' ? 'b-amber' : d.urg === 'green' ? 'b-green' : d.urg === 'blue' ? 'b-blue' : 'b-gray';
      return \`<div class="div-card" style="margin-bottom:.75rem">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;margin-bottom:.5rem">
          <div>
            <div class="div-nome">\${d.nome} <span class="badge \${bc}">\${d.tipo === 'picpay' ? 'Folha' : 'Em aberto'}</span></div>
            <div class="div-meta">Parcela: <strong style="color:var(--accent)">\${fmt(d.parcMensal)}</strong> · Vence: \${venceStr} · Restam \${parcsRest} de \${d.parcsTotal || 0} parcelas</div>
          </div>
...
window.pagarParcela = async (divId) => {
  const div = dividas.find(d => d.id === divId);
  if (!div) return;
  const val = div.parcMensal;
  const data = today();
  const novoPago = (div.pago || 0) + val;
  const novoTotal = Math.max(0, div.total - val);
  const novasParcsPagas = (div.parcsPagas || 0) + 1;
  const updates = { pago: novoPago, total: novoTotal, parcsPagas: novasParcsPagas };
  if (novoTotal <= 0.01) { updates.quitada = true; updates.dataQuit = data; }
  try {
    await updateDoc(doc(db, 'users', currentUser.uid, 'dividas', divId), updates);
    await addDoc(collection(db, 'users', currentUser.uid, 'transacoes'), {
      tipo: 'pag-divida', divida: div.nome, label: 'Parcela — ' + div.nome, val, data, isParcela: true, criadoEm: Date.now()
    });
    const parcsRest = (div.parcsTotal || 0) - novasParcsPagas;
    if (novoTotal <= 0.01) toast('🎉 ' + div.nome + ' QUITADA!');
...
function renderDivCard(d) {
  ...
  const parcsRest = Math.max(0, (d.parcsTotal || 0) - (d.parcsPagas || 0));
  const parcsInfo = d.parcMensal ? `Parcela mensal: <strong style="color:var(--accent)">${fmt(d.parcMensal)}</strong> · ${parcsRest} de ${d.parcsTotal || 0} restantes` : '';
    ...
    <div class="div-card${d.negociada ? ' negociada' : ''}${d.tipo === 'picpay' ? ' picpay' : ''}">
      <div class="div-header">
        <div style="flex:1">
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:2px">${parcsInfo}</div>
          ${novaInfo}${ppInfo}
          ${pago > 0 ? `<div style="font-size:12px;color:var(--accent);margin-top:2px">Pago: ${fmt(pago)}</div>` : ''}
        </div>
        <div class="div-actions">
          <button class="btn btn-g btn-sm" onclick="pagarParcela('${d.id}')">Pagar parcela</button>
